...
\address{%
$^{1}$ \quad auto-res2 / lean-summation-identities-2 (GitHub)}
...
Both claims were realized from the imported reports and both are \emph{supported}; the verdicts appear in the claim list of Section~5, which is regenerated from the record at every verification. For each run the report shows an empty error list, no warnings (so no \texttt{sorry}), and a built type identical to the frozen statement up to whitespace: the declarations were built for exactly the types that were preregistered, and neither claim needed to be appended again.
...
\paragraph{What broke, and what it says about the flow.} Two failures occurred before the reported runs, both outside the Lean pipeline. The repository's Python lockfile shipped out of sync with its project file, so the Docker image failed to build and the workflow fell back to native execution, where no Lean toolchain exists; the first pair of runs failed there. Refreshing the lockfile fixed it. Separately, the journal template's logo is shipped as EPS, which the non-shell-escape build in CI cannot convert, so the first paper build on the protected branch failed until a converted logo was added. Neither failure could have produced a wrong verdict --- a failed run simply has no report --- but both show that the execution environment is part of the claim's reproducibility and should be checked before a proof is written.
...
Two summation identities were preregistered as Lean claims, proved after the freeze, built in an isolated GitHub Actions run, and reported only through the record realized from the run's report. Both claims are supported, with the built types equal to the frozen statements and the axioms within the declared set. The AIRAS integrity flow, built for experiments, carried a theoretical claim end to end without changes to its verification rules; the residual work is on the edges --- semantic rather than textual comparison of the frozen statement, and a gate that re-derives the report by rebuilding the proof.
...
\dataavailability{All resources used in this study are openly available at \url{https://github.com/auto-res2/lean-summation-identities-2}.}
